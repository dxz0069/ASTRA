#!/usr/bin/env python3
"""赛后记忆蒸馏流水线（"睡眠巩固"）：一轮跑完 → 自动产出三件套草稿，人只审核。

产出（docs/review-drafts/<日期>/）：
...
def _load_pending(name: str) -> dict:
...
def main() -> int:
    ...
    pending = _load_pending("astra-knowledge-append.json")
    dd_pending = _load_pending("astra-deadends-append.json")
    stats = {}
    try:
    ...
    print(f"[done] 三件套草稿已生成 → {out_dir}")
    print(f"  新沉淀 {len(pending)} ｜ 死路 {len(dd_pending)} ｜ 模式：{mode}")
    return 0
