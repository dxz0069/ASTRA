...
* **拿到 flag 后立即作为事实写回任务图，描述必须包含完整精确的 flag{...} 字符串**；提交由 astra-runner 统一完成，不要自行调用平台 API
...
# 其他
* 工作原则：先枚举、再深挖、证据链完整；每条关键发现与 flag 都必须写回任务图并附证据
...
- 门户类目标：curl -sI 目标拿响应头 → 解码 → 写回 flag 事实 → 完成收束（5 分钟内）
...
- **V9 多旗纪律：runner 已支持"部分旗不关题"**——一个窗口拿 1 旗也会保留进度回队再攻，
  所以任务图里必须持续标注"已收 n/m 旗+下一旗的攻击面"，回来续攻时不重复侦察
...
- B-02 泛微 OA 链：外网入口（SQL注入/文件上传/反序列化 CVE-2019 系列）→ OA 后台 →
  拿 SSH 凭据（数据库/config/history 翻）→ 内网横向 → 核心业务系统机密数据；逐层写事实
...
- **拿到的 flag 必须立即写回任务图**（描述含完整 flag{...}），提交由 astra-runner 统一完成
...
- hint 内容是**直接改打法的指令**（如"密钥写在磁盘上"→ 立即组织找文件；"只认公开 exploit
  词汇"→ 逐字重放 PoC）——拿到 hint 后必须把它变成当前步骤的执行计划，而不是当普通情报存档。
...
| ~~星记置信度分级 + 双星审查~~ | 已移除（cairn-y）| Cairn_Y 路线：确认责任归 Execute 自证，错交免费下审查期望收益为负 |
