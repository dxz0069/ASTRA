...
* **拿到 flag 后立即作为天枢写回星图，描述必须包含完整精确的 flag{...} 字符串**；提交由 astra-runner 统一完成，不要自行调用平台 API
...
- 门户类目标：curl -sI 目标拿响应头 → 解码 → 写回 flag 天枢 → 归航（5 分钟内）
...
- B-02 泛微 OA 链：外网入口（SQL注入/文件上传/反序列化 CVE-2019 系列）→ OA 后台 →
  拿 SSH 凭据（数据库/config/history 翻）→ 内网横向 → 核心业务系统机密数据；逐层写天枢
...
- hint 内容是**直接改打法的指令**（如"密钥写在磁盘上"→ 立即组织找文件；"只认公开 exploit
  词汇"→ 逐字重放 PoC）——拿到 hint 后必须把它变成当前斗柄的执行计划，而不是当普通情报存档。

# Harness 工程纪律：控制三分法（面向 harness 开发；解题 worker 可忽略）

> 出处：两届 TCH 决赛公开复盘（2026-05）× 三 AI 评审收敛信号——见 `docs/两届TCH决赛方案AI评审拆解.md`。
...
| ~~天枢置信度分级 + 双星审查~~ | 已移除（v0.2）| 星图路线：确认责任归 Execute 自证，错交免费下审查期望收益为负 |
