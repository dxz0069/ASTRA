...
def test_text_file_archive_extracts_below_existing_top_level_directory() -> None:
    archive_path, payload = ContainerManager._text_file_archive(
        "/tmp/astra-prompts/reason_execute-123/graph.yaml",
        "facts:\n- id: f001\n",
    )
    ...
    with tarfile.open(fileobj=io.BytesIO(payload)) as archive:
        ...
        assert names == [
            "astra-prompts",
            "astra-prompts/reason_execute-123",
            "astra-prompts/reason_execute-123/graph.yaml",
        ]
        assert "tmp" not in names
        graph = archive.extractfile("astra-prompts/reason_execute-123/graph.yaml")
