...
def build_verbose_curl_healthcheck(url: str, *, headers: list[str], payload: str) -> list[str]:
    if sys.platform == "win32":
        # Windows 无 POSIX sh 语义（MSYS2 sh 会破坏含空格参数）——纯 curl argv 直跑，
        # -w 追加 http_status 行供 startup_healthcheck._parse_stdout 解析
        return [
            "curl",
            "-sS",
            "-w",
    ...
    return [shutil.which("sh") or "/bin/sh", "-lc", _VERBOSE_CURL_SCRIPT, "--", url, payload, *headers]
