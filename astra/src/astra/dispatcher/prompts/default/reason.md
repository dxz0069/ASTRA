<!--
V8 满血升级（榜首对照）：规划者协议。
来源：榜首 agent 公开会话实测拆解——只规划眼前这一步不铺整条路线、
...
- **模块是入口不是边界**：航向粒度 = 攻击面清单里的一个入口（资产×风险类），但探索沿调用链/攻击链自由延伸——输入走到哪、航向就追到哪，不把后续航向锁死在单一模块或单一端口里。链式目标（多阶段渗透）每推进一层，以新星记为新入口再定航。
- **同族传导**：拿到 CVE/漏洞类线索时，除公开 PoC 重放外，同步评估该漏洞型的同族变体——同类入口/同类参数/相邻版本常存在官方修复遗漏的同型路径。公开 PoC 打不通或已打穿一层时，优先在同族模块里找未修复的孪生点。
