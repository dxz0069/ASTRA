...
引擎基于**星图协议**：所有状态外部化为一张 append-only 的因果图（事实—目标—步骤），
...
安全 Agent 不该是"金鱼"：ASTRA 的记忆管线让经验形成闭环——

- **四层记忆**：瞬时（会话原始上下文）→ 工作记忆（焦点子图裁剪）→ 短时（星图事实库 + Epitome
  自动摘要压缩）→ 长时（攻击链沉淀、双层脱敏、开局注入）
- **进化机制**：语义召回（词面+向量混合，`ASTRA_EMBED_API_KEY` 开启，向量 SQLite 持久化跨会话复用）、
  回忆强化/遗忘曲线（战绩 × 半衰期 30 天衰减）、举一反三（同题型按战绩加权注入）、赛中实时复利
  （解出即沉淀热加载）、失败经验库（死路避坑）、赛后自动蒸馏（三件草稿自动产出，人工只审核）
- **质询星探**：decide 的 complete 收束与凭据/flag 级关键事实须经独立对抗审查方可入图
  （fail-open：质询故障绝不卡主链路；`ASTRA_CHALLENGE_MODE=0` 可关）
- **观测**：`astra memory stats / trace / map`——四层全景、决策链一键回放、星图可视化

相关开关：`ASTRA_EPITOME`（图压缩，阈值 `ASTRA_EPITOME_THRESHOLD`）、`ASTRA_AUTO_DISTILL`
（赛后蒸馏）、`ASTRA_EMBED_PERSIST`（向量持久化）；本地默认全开，托管镜像默认关。
...
| **摇光（Execute）** | 认领一条斗柄，干完它 | 一条天枢（自证写回，可携客星） |
