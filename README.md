...
| 原语 | 含义 |
|------|------|
| **星记（Fact）** | 一条已确认的客观发现（当前世界状态；负结果同样是星记） |
| **北辰（Goal）** | 项目的完成条件 = 搜索的终止条件；可挂动态**星站（SubGoal）** |
| **航向（Step）** | 从既有星记出发、预期产出新星记的因果行动；可关闭并留痕 |
| **星辉（Finding）** | 搜索过程的沿途产出（如漏洞）——与终点相对的一等公民 |
| **指引（Hint）** | 任意时刻注入的人类判断，巡猎者下次读取时吸收 |

## 双活动架构（Decide & Execute）

引擎只有两类活动，它们是**同一个运行器注入不同提示词与工具**的实例，而非固定角色的子智能体：

- **定航（Decide）**：串行、事件触发（任务开始或图变化）、每次从干净上下文起跑。
  只拥有图操作权限——判定北辰是否抵达（终止搜索）、开辟航向（必附"预期产出什么星记"）、
  关闭失效航向（留痕防重开死路）、增删星站。
- **巡猎（Execute）**：并发多实例。拥有世界工具（read/bash/edit/write）与唯一的
  `submit_fact` 入图闸口——确认责任在巡猎者自证，跑过命令、亲眼看到输出才写回，
  可携一道星辉。
...
- 每次任务只内联与当前北辰最相关的星记与航向（相关度 + 语义召回 + 时间近度 + 图距加成，
  凭据级关键星记钉住保底），受 `context_budget` 硬上限约束
- 完整星图仅作文件引用提供（巡猎者自行按需读取）
...
| 任务 | 做什么 | 产出 |
|------|--------|------|
| **首探（Bootstrap）** | 项目启动时的首次巡猎：直接从起点开工 | 流式星记 + 可能的归航 |
| **定航（Decide）** | 读星图：北辰抵达了吗？下一批航向怎么走？ | 归航 / 新航向+关航向+星站 / 无操作 |
| **巡猎（Execute）** | 认领一条航向，干完它 | 一条星记（submit_fact，可携星辉） |
